 ...
 *
 * Every finding the tool can raise is declared once: which audit type it
 * belongs to, which scored parameter it pulls down, how hard it hits, what it
 * costs the business, and which DigiKraft service picks it up. Analysers supply
 * only the measured evidence, so the sales language stays in one place.
 */
final class Rules
{
    /** Points knocked off a parameter's 10, by severity. */
    public const WEIGHT = ['high' => 4, 'medium' => 2, 'low' => 1];

    /**
     * @return array<string,array{type:string,parameter:string,severity:string,title:string,cost:string,service:string,month:string}>
     */
    public static function all(): array
    {
        return array_merge(self::landing(), self::social(), self::document(), self::branding());
    }
    ...
    private static function landing(): array
    {
        ...
        return [
            // ---------------------------------------------------- value proposition
            'lp-no-h1' => [
                'type' => 'landing', 'parameter' => 'value-prop', 'severity' => 'high',
                ...
                'service' => $web, 'month' => 'Month 1',
            ],
            'lp-multi-h1' => [
                'type' => 'landing', 'parameter' => 'value-prop', 'severity' => 'medium',
            ...
            'lp-headline-title-mismatch' => [
                'type' => 'landing', 'parameter' => 'value-prop', 'severity' => 'low',
            ...
            // ------------------------------------------------------------------ CTA
            'lp-no-cta' => [
                'type' => 'landing', 'parameter' => 'cta', 'severity' => 'high',
            ...
            // ------------------------------------------------------------------- UX
            'lp-nav-overload' => [
                'type' => 'landing', 'parameter' => 'ux', 'severity' => 'medium',
            ...
            'lp-link-density' => [
                'type' => 'landing', 'parameter' => 'ux', 'severity' => 'low',
            ...
            'lp-no-lazyload' => [
                'type' => 'landing', 'parameter' => 'speed', 'severity' => 'low',
            ...
            'lp-small-type' => [
                'type' => 'landing', 'parameter' => 'mobile', 'severity' => 'low',
            ...
            'lp-no-contact' => [
                'type' => 'landing', 'parameter' => 'trust', 'severity' => 'low',
    ...
    {
        $svc = 'Social & content design';
        return [
            // ------------------------------------------------- profile optimisation
            'sm-no-bio' => [
                'type' => 'social', 'parameter' => 'profile-optimization', 'severity' => 'high',
                'title' => '{network} has no bio',
                'cost'  => 'The first thing a new visitor reads is blank.',
                'service' => $svc, 'month' => 'Month 1',
            ],
            'sm-thin-bio' => [
                'type' => 'social', 'parameter' => 'profile-optimization', 'severity' => 'medium',
            ...
            'sm-network-gap' => [
                ...
                'service' => $svc, 'month' => 'Month 2',
            ],

            // ----------------------------------------------------------- engagement
            'sm-low-engagement' => [
                'type' => 'social', 'parameter' => 'engagement', 'severity' => 'high',
                'title' => '{network} averages {rate} engagement per post',
                'cost'  => 'The content is being published, not received. Paid cannot lean on it.',
                'service' => $svc, 'month' => 'Month 2',
            ],
            'sm-flat-reach' => [
                'type' => 'social', 'parameter' => 'engagement', 'severity' => 'medium',
            ...
            // ---------------------------------------------------- visual consistency
            'sm-title-drift' => [
                'type' => 'social', 'parameter' => 'visual-consistency', 'severity' => 'medium',
    ...
    private static function document(): array
    {
        ...
        return [
            ...
            'doc-long-sentences' => [
                'type' => 'document', 'parameter' => 'clarity', 'severity' => 'medium',
            ...
            'doc-no-contact' => [
                'type' => 'document', 'parameter' => 'actionability', 'severity' => 'medium',
            ...
            'doc-spacing' => [
                'type' => 'document', 'parameter' => 'accuracy', 'severity' => 'low',
            ...
            // -------------------------------------------------------- visual design
            'doc-font-sprawl' => [
                'type' => 'document', 'parameter' => 'visual-design', 'severity' => 'medium',
            ...
            'doc-no-title-meta' => [
                'type' => 'document', 'parameter' => 'visual-design', 'severity' => 'low',
    ...
    private static function branding(): array
    {
        ...
        return [
            // ------------------------------------------------------ visual identity
            'br-multi-logo' => [
                'type' => 'branding', 'parameter' => 'visual-identity', 'severity' => 'high',
            ...
            'br-no-logo' => [
                'type' => 'branding', 'parameter' => 'visual-identity', 'severity' => 'medium',
            ...
            // ---------------------------------------------------------- brand voice
            'br-tone-drift' => [
                'type' => 'branding', 'parameter' => 'brand-voice', 'severity' => 'medium',
            ...
            // --------------------------------------------------- audience alignment
            'br-audience-absent' => [
                'type' => 'branding', 'parameter' => 'audience-alignment', 'severity' => 'high',
            ...
            'br-audience-thin' => [
                'type' => 'branding', 'parameter' => 'audience-alignment', 'severity' => 'medium',
            ...
            // ----------------------------------------------------------- uniqueness
            'br-cliche' => [
                'type' => 'branding', 'parameter' => 'uniqueness', 'severity' => 'medium',
            ...
            'br-type-overlap' => [
                'type' => 'branding', 'parameter' => 'uniqueness', 'severity' => 'low',
            ...
            // ---------------------------------------------------------- core values
            'br-no-mission' => [
                'type' => 'branding', 'parameter' => 'core-values', 'severity' => 'high',
    ...
    /**
     * Build a finding from a rule id plus measured evidence.
     *
     * @param  array<string,string|int|float> $tokens replaced into the title
     * @param  array<int,array{label:string,value:string,url?:string}> $proof
     * @return array<string,mixed>
     */
    public static function make(string $id, string $evidence, array $tokens = [], array $proof = [], ?string $severity = null): array
    {
        $rules = self::all();
        if (!isset($rules[$id])) {
            throw new \InvalidArgumentException('Unknown rule: ' . $id);
        }
        $rule  = $rules[$id];
        $title = $rule['title'];
        $leads = strpos($title, '{') === 0;
        ...
        return [
            ...
            'month'     => $rule['month'],
            'proof'     => array_values($proof),
        ];
    }

    /** Spell small numbers. Lowercase — make() capitalises leading words. */
    ...
    public static function plural(int $n, string $singular, ?string $plural = null): string
    {
