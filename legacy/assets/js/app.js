...
(function () {
  ...
  var BOOT    = window.DK_BOOT || { endpoint: "api.php", report: "report.php" };
  var TYPES   = window.DK_TYPES || {};
  var STORAGE = "dk-cga-session-v3";
  ...
  function api(action, options) {
    options = options || {};
    var url  = BOOT.endpoint + "?a=" + encodeURIComponent(action);
    ...
    if (options.query) {
      Object.keys(options.query).forEach(function (k) {
        url += "&" + encodeURIComponent(k) + "=" + encodeURIComponent(options.query[k]);
      });
    }
    if (options.form) {
      init.method = "POST";
  ...
  function reportUrl() {
    return BOOT.report + "?id=" + encodeURIComponent(state.id) + "&t=" + encodeURIComponent(state.token);
  }
